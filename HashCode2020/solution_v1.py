...
choise = 5 # from 0 to 5

class Library():

  lib_index = 0 # indexing libs
  time_coefficient = 6100 # coefficient in experimental part

  def __init__(self, books_count, signup_time, books_perday, books):
  ...
  def calc_priority(self, nday):
    if nday - self.time <= 0:   # if signup > then rest of days.
      ...
      # ----
      if b2p < self.count:
        self.priority = (costs[:b2p].sum() / self.count) / self.time / 0.5
      else:
        #self.priority = costs[:b2p].sum() - (self.time * Library.time_coefficient)
        self.priority = (costs[:b2p].sum() / self.count) / self.time
      # ----
...
# solution ------------------------------------------------------------------
print("Set: {}".format(["a", "b", "c", "d", "e", "f"].pop(choise)))
